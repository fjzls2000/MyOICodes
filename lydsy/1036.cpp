...
{
    int k=0;sz++;
    pos[x]=sz;//·ÖÅäx½áµãÔÚÏß¶ÎÊ÷ÖÐµÄ±àºÅ
    bl[x]=chain;
    for(int i=head[x];i;i=e[i].next)
        if(dep[e[i].to]>dep[x]&&size[e[i].to]>size[k])
            k=e[i].to;//Ñ¡Ôñ×ÓÊ÷×î´óµÄ¶ù×Ó¼Ì³ÐÖØÁ´
    if(k==0)return;
    dfs2(k,chain);
    for(int i=head[x];i;i=e[i].next)
        if(e[i].to != fa[x]&&k!=e[i].to)
            dfs2(e[i].to,e[i].to);//ÆäÓà¶ù×ÓÐÂ¿ªÖØÁ´
}
void build(int k,int l,int r)//½¨Ïß¶ÎÊ÷
{
    t[k].l=l;t[k].r=r;
    if(l==r)return;
    int mid=(l+r)>>1;
...
void change(int k,int x,int y)//Ïß¶ÎÊ÷µ¥µãÐÞ¸Ä
{
...
int querysum(int k,int x,int y)//Ïß¶ÎÊ÷Çø¼äÇóºÍ
{
    int l=t[k].l,r=t[k].r,mid=(l+r)>>1;
...
int querymx(int k,int x,int y)//Ïß¶ÎÊ÷Çø¼äÇó×î´óÖµ
{
